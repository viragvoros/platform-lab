...
Prerequisites: docker, kind, kubectl. See [docs/setup.md](docs/setup.md) for the full install list, including linters used for local development.
...
After ~3 minutes Argo CD will sync the example `hello` workload. Verify:

```bash
kubectl get applications -n argocd     # both 'root' and 'hello' Synced/Healthy
